...
目前 version 為 `1`。Restore 時必須同時驗證 allowlist、format/version、SHA-256 與 SQLite `PRAGMA integrity_check`；任何不相容或損壞都 fail closed。
...
Restore 採原子 replace，因此重複 restore 同一 artifact 不會重新建立 task/run/event/artifact identity。

## Security boundary

Portability layer 不讀取 ambient credentials，也不額外掃描 filesystem。Artifact 只包含 Runtime SQLite database 的內容；若未來 Runtime schema 開始保存 secret，必須先新增明確 secret filtering / encryption contract，不能沿用 v0 自動匯出。
