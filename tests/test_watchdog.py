...
def test_repeated_idle_watchdog_is_zero_noise(tmp_path: Path) -> None:
    ...
    init_db(db)
    before = _counts(db)
    first = watchdog_wake(path=db)
    second = watchdog_wake(path=db)
...
def test_watchdog_limit_is_bounded_and_deterministic(tmp_path: Path) -> None:
    db = tmp_path / "runtime.db"
    init_db(db)
    events = [_safe_event(db, f"event-{index}") for index in range(3)]
    first = watchdog_wake(dispatch_limit=2, path=db)
    assert first["dispatch"]["scanned"] == 2
    ...
    assert second["dispatch"]["scanned"] == 1
...
def test_fresh_restore_watchdog_recovers_missed_ingress(tmp_path: Path) -> None:
    ...
    init_db(source)
    event = _safe_event(source, "portable-missed")
